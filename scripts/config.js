...
/* ═══ «نقشهٔ رفرنس» (Reference Drawing) ═══
   نقشه‌ای که مشتری همراهِ درخواستِ ساخت می‌فرستد و بعداً در فرمتِ فولاد شرق دوباره
   طراحی می‌شود. نامش عمداً همان چیزی است که در جدولِ پایینِ نقشه‌های مهندسی، خانهٔ
   «نقشهٔ رفرنس» نوشته شده — تا بینِ کاغذ و سامانه ترجمهٔ ذهنی لازم نباشد.
   این سند شمارهٔ خودش را دارد (شمارهٔ مشتری) که با شمارهٔ سامانه فرق می‌کند؛ تنها
   جای ثبتِ آن، فیلدِ توضیحاتِ همین سند است و جست‌وجوی آرشیو هم روی همان کار می‌کند،
   پس برای این نوع پرکردنِ توضیحات اجباری شده است. */
var RD_CODE = "RD";
var RD_NOTE_PH = "شمارهٔ نقشهٔ مشتری — مثال: DWG-4471-B";
var RD_NOTE_HINT = "شمارهٔ نقشهٔ اصلی مشتری را اینجا بنویسید — همان شماره‌ای که در جدول نقشه، خانهٔ نقشهٔ رفرنس به آن ارجاع می‌دهد.";
var RD_INFO_IC = '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="12.5"/><circle cx="12" cy="16" r=".6" fill="currentColor" stroke="none"/></svg>';
/* آیا نوعِ سندِ «نقشهٔ رفرنس» در فهرستِ انواعِ سند تعریف شده؟ اگر کاربر هنوز اضافه‌اش
   نکرده باشد، هیچ ماژول یا اجباری نباید ساخته شود (وگرنه ماژولی بی‌برچسب می‌ماند). */
function rdTypeReady(){
  return (typeof DB!=="undefined" && DB && DB.docTypes || []).some(function(x){
    return String(x.code).toUpperCase()===RD_CODE && String(x.scope||"")!=="project";
  });
}
